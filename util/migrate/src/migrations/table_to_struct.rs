...
impl ChangeMoleculeTableToStruct {
    ...
    fn migrate_header(&self, db: &RocksDB) -> Result<()> {
        ...
        while !next_key.is_empty() {
            ...
            let (_count, nk) =
                db.traverse(COLUMN_BLOCK_HEADER::NAME, &mut header_view_migration, mode, LIMIT)?;
            next_key = nk;
    ...
    fn migrate_uncles(&self, db: &RocksDB) -> Result<()> {
        ...
        while !next_key.is_empty() {
            ...
            let mode = self.mode(&next_key);
            let (_count, nk) = db.traverse(COLUMN_UNCLES::NAME, &mut uncles_migration, mode, LIMIT)?;
            next_key = nk;
    ...
    fn migrate_transaction_info(&self, db: &RocksDB) -> Result<()> {
        ...
        while !next_key.is_empty() {
            ...
            let (_count, nk) =
                db.traverse(COLUMN_UNCLES::NAME, &mut transaction_info_migration, mode, LIMIT)?;
            next_key = nk;
    ...
    fn migrate_epoch_ext(&self, db: &RocksDB) -> Result<()> {
        ...
        while !next_key.is_empty() {
            ...
            let mode = self.mode(&next_key);
            let (_count, nk) = db.traverse(COLUMN_EPOCH::NAME, &mut epoch_ext_migration, mode, LIMIT)?;
            next_key = nk;
...
impl Migration for ChangeMoleculeTableToStruct {
    fn migrate(
        ...
        let mut wb = db.new_write_batch();
        if let Some(current_epoch) = db.get_pinned(COLUMN_META::NAME, COLUMN_META::META_CURRENT_EPOCH_KEY)? {
            if current_epoch.len() != 108 {
                wb.put(COLUMN_META::NAME, COLUMN_META::META_CURRENT_EPOCH_KEY, &current_epoch[36..])?;
            }
        }
        db.write(&wb)?;
