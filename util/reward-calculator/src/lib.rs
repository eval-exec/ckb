...
impl<'a, CS: ChainStore> RewardCalculator<'a, CS> {
    ...
    fn proposal_reward(
        ...
        let mut target_proposals = self.get_proposal_ids_by_hash(target.number(), &target.hash());
        ...
        while index.number() > competing_commit_start && !target_proposals.is_empty() {
            ...
            let previous_ids = store
                ...
                .map(|hash| self.get_proposal_ids_by_hash(competing_proposal_start, &hash))
    ...
    fn get_proposal_ids_by_hash(
        ...
        if let Some(ids) = self.store.get_block_proposal_txs_ids(number, hash) {
            ids_set.extend(ids)
        }
        if let Some(us) = self.store.get_block_uncles(number, hash) {
