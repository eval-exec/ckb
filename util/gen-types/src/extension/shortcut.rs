...
/// Type alias for stack-allocated block-number key
...
/// Creates a RocksDB key from a block number.
///
/// Format: `Uint64 (block_number, big-endian)`.
/// Big-endian encoding preserves numeric ordering in RocksDB's bytewise comparator.
pub fn block_number_to_key(number: BlockNumber) -> BlockNumberKey {
    number.to_be_bytes()
}

/// Extracts a block number from a RocksDB block-number key.
pub fn block_number_from_key(key: &[u8]) -> Option<BlockNumber> {
    if key.len() != BLOCK_NUMBER_KEY_SIZE {
        return None;
    }
    Some(BlockNumber::from_be_bytes(key.try_into().ok()?))
}
